...
```shell
cd server/frontend/ && SHOPIFY_API_KEY=REPLACE_ME yarn build
```

Using npm:

```shell
cd server/frontend/ && SHOPIFY_API_KEY=REPLACE_ME npm run build
```

Using pnpm:

```shell
cd server/frontend/ && SHOPIFY_API_KEY=REPLACE_ME pnpm run build
```
